...
- **Sidebar entry**: registers an optional Workbench `top` row backed by the existing board controller; when the Sidebar owner does not expose compatible row slots, it keeps the rc.6 fallback entry injected inside the sidebar column (`[data-pane="sidebar"]` on older shells, `[class*="sidebarCol"]` on the DSH 0.1.0-rc.6 AppFrame layout) below the new-session button.
...
- **Sidebar compatibility**: the shipped rc.6 Sidebar declares `sidebar.workspaces` / `sidebar.settings` single slots and `sidebar.footer.action`, but no top-row slot. When an optional Workbench service and compatible Sidebar row owner are present, this plugin registers one declaration-aware top row whose summary, expanded state, and toggle reuse the existing controller. Otherwise the legacy entry uses self-healing DOM injection as a fail-soft fallback; scheduling and execution ownership never move into Workbench.
