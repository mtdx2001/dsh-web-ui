...
export function readUiState(root: string): WorkbenchUiState {
  ...
  const activeRightPanel = typeof stored.activeRightPanel === 'string' && /^(?:[a-z][a-z0-9._/-]*:)?[a-z][a-z0-9-]*$/.test(stored.activeRightPanel)
    ? (stored.activeRightPanel === 'overview' ? 'workbench:overview' : stored.activeRightPanel)
    : 'workbench:overview'
  return {
