...
async function fetchGitStatus(root: string, signal: AbortSignal): Promise<GitStatusLike | null | undefined> {
  try {
    const response = await fetch('/dsh-workbench/git-status', {
      method: 'POST',
...
export function startController(ctx: ClientContext, stores: WorkbenchStores, componentPreferences?: ComponentPreferencesService): () => void {
  ...
  const syncSession = (): void => {
    ...
    componentPreferences?.setRoot(root)
