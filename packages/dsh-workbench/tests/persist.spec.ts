...
describe('per-project UI persistence', () => {
  ...
  it('round-trips the UI state under the project key', () => {
    writeUiState('/repo/a', { overviewActive: true })
    expect(localStorage.getItem(`${KEY_UI_PREFIX}/repo/a`)).toBe('{"overviewActive":true}')
    expect(readUiState('/repo/a')).toEqual({ overviewActive: true })
  })
  ...
  it('falls back to defaults for missing, broken, or mistyped records', () => {
    ...
    expect(readUiState('/repo/mistyped')).toEqual({ overviewActive: false })
  ...
  it('never writes the empty root', () => {
    writeUiState('', { overviewActive: true })
